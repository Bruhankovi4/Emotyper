...
namespace DSP
{
    public class MFCC
    {
        public static double[] melWorkingFrequencies = new double[10] { 12.68, 13.91, 15.14, 16.37, 17.6, 18.83, 20.06, 21.29, 22.52, 23.76 };

        public static void initWithrange(double start, double end, int count)
        {
            melWorkingFrequencies = new double[count];
            double interval = end - start;
            double step = interval / (count + 1);
            for (int i = 0; i < count; i++)
            {
                melWorkingFrequencies[i] = start + i * step;
            }
        }
        ...
        public static double[] compute(ref double[] signal)
        {
            ...
            System.Diagnostics.Debug.WriteLine("segment:"+Convert.ToString(segment));*/http://vk.com/feed

                start = (segment - (int)Math.Floor((double)segment / 2));
