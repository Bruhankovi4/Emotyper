...
namespace ExampleWithGraphics
{
    ...
    public partial class MainWindow : Window
    {
        ...
        void MakeCircle(int n, List<Vertex> vertices)
        {
            ...
            //}
            string path = "D://GitRepos//Emotyper//Emotyper//SOMCoordinates//AF3Coordinates.csv";
            DataTable tableSource = CSVReader.ReadCSVFile(path.Replace("\\", "//"), false);

          //  List<Vertex> points = new List<Vertex>();
            for (int i = 0; i < tableSource.Rows.Count; i++)
            {
                vertices.Add(new Vertex(Double.Parse(tableSource.Rows[i][0].ToString()) / 100 * sizeX, Double.Parse(tableSource.Rows[i][1].ToString()) / 100 * sizeY));
            }
        }
